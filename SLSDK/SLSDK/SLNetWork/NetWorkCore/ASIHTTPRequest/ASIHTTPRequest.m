...
static NSError *ASIVersionError;
...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
        ...
        ASIVersionError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIVersionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it version too low",NSLocalizedDescriptionKey, nil]];
...
- (void)failWithError:(NSError *)theError
{
	...
	if (theError && [theError code] != ASIAuthenticationErrorType && [theError code] != ASITooMuchRedirectionErrorType && [theError code] != ASIVersionErrorType) {
		[connectionsLock lock];
		#if DEBUG_PERSISTENT_CONNECTIONS
...
- (void)readResponseHeaders
{
    ...
    } else if ([self responseStatusCode] == 406) {
        [self failWithError:ASIVersionError];
    } else {
		#if DEBUG_HTTP_AUTHENTICATION
